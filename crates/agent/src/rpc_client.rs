...
impl ImFileBridge for GrpcImFileBridge {
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<DownloadedImFile>> + Send + 'a>>
    {
        Box::pin(async move {
            ...
            let response = self
                .request(
                    &reply_to,
                    remi_proto::im_bridge_request::Payload::Download(
                        remi_proto::ImDownloadRequest {
            ...
            match response.payload {
                Some(remi_proto::im_bridge_response::Payload::Downloaded(file)) => {
                    Ok(DownloadedImFile {
                ...
                Some(_) => Err(anyhow!(
                    "unexpected IM bridge response payload for download"
                )),
    ...
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<UploadedImFile>> + Send + 'a>>
    {
        Box::pin(async move {
            ...
            match response.payload {
                Some(remi_proto::im_bridge_response::Payload::Uploaded(file)) => {
                    Ok(UploadedImFile {
                        file_name: file.file_name,
